    ...
    return new Promise((resolve, reject) => {
        ...
        files.forEach((file, index) => {
            ...
            upload.on('httpUploadProgress', (progress) => {
                ...
                // Here you can use sockets to send progress updates to the client if needed
            });

            upload.send((err, data) => {
                if (err) {
                    console.log(`Error uploading ${fileName}:`, err);
                    uploadResults[index] = { file: fileName, status: 'error', error: err };
                } else {
                    console.log(`Successfully uploaded ${fileName}`);
                    uploadResults[index] = { file: fileName, status: 'success', location: data.Location };
                }
                completedFiles++;
...
app.post('/upload', upload.array('media_h', 10), async (req, res) => {
    try {
        ...
        const uploadResults = await uploadToS3(webpFilesArray);
        res.json({ success: true, message: 'Files uploaded', data: uploadResults });
    } catch (err) {
        console.log(`Error in upload:`, err);
        res.status(500).json({ success: false, message: 'File upload failed', error: err });
    }
});
